...
export class AnalyticsService {
  static async recordClick(clickData) {
    try {
      ...
      const page = await prisma.page.findUnique({
        where: { id: clickData.pageId },
        select: { userId: true }
      });

      if (!page) {
        throw new Error(`Page not found: ${clickData.pageId}`);
      }
      ...
      await this.updateDailyAnalytics(clickData.pageId, page.userId, new Date(), clickData);
      
      logger.info('Click recorded successfully:', click.id);
      return click;
    } catch (error) {
  ...
  static async updateDailyAnalytics(pageId, userId, date, clickData) {
    try {
      // Normalize date to start of day
      const dateStart = new Date(date);
      dateStart.setHours(0, 0, 0, 0);
      
      // Determine if this is a page view (no blockId) or block click
      const isPageView = !clickData.blockId;
      
      // Get or create analytics record for this user and date
      const analytics = await prisma.analytics.upsert({
        ...
        update: {
          // Increment pageViews if this is a page view
          pageViews: isPageView ? { increment: 1 } : undefined,
          // Increment totalClicks for all clicks
          totalClicks: { increment: 1 },
          // Update device stats if this is a page view
          ...(isPageView && clickData.device && {
            mobileViews: clickData.device === 'mobile' ? { increment: 1 } : undefined,
            desktopViews: clickData.device === 'desktop' ? { increment: 1 } : undefined,
            tabletViews: clickData.device === 'tablet' ? { increment: 1 } : undefined,
          }),
          // Update traffic source stats if this is a page view
          ...(isPageView && {
            directTraffic: !clickData.referer ? { increment: 1 } : undefined,
            socialTraffic: clickData.referer?.includes('facebook') || 
                          clickData.referer?.includes('twitter') || 
                          clickData.referer?.includes('instagram') || 
                          clickData.referer?.includes('linkedin') ? { increment: 1 } : undefined,
            searchTraffic: clickData.referer?.includes('google') || 
                          clickData.referer?.includes('bing') || 
                          clickData.referer?.includes('yahoo') ? { increment: 1 } : undefined,
            referralTraffic: clickData.referer && 
                            !clickData.referer.includes('facebook') && 
                            !clickData.referer.includes('twitter') && 
                            !clickData.referer.includes('google') && 
                            !clickData.referer.includes('bing') ? { increment: 1 } : undefined,
          })
        },
        create: {
          userId: userId,
          ...
          revenue: 0,
          mobileViews: isPageView && clickData.device === 'mobile' ? 1 : 0,
          desktopViews: isPageView && clickData.device === 'desktop' ? 1 : 0,
          tabletViews: isPageView && clickData.device === 'tablet' ? 1 : 0,
          directTraffic: isPageView && !clickData.referer ? 1 : 0,
          socialTraffic: isPageView && (clickData.referer?.includes('facebook') || 
                                        clickData.referer?.includes('twitter') || 
                                        clickData.referer?.includes('instagram') || 
                                        clickData.referer?.includes('linkedin')) ? 1 : 0,
          searchTraffic: isPageView && (clickData.referer?.includes('google') || 
                                       clickData.referer?.includes('bing') || 
                                       clickData.referer?.includes('yahoo')) ? 1 : 0,
          referralTraffic: isPageView && clickData.referer && 
                          !clickData.referer.includes('facebook') && 
                          !clickData.referer.includes('twitter') && 
                          !clickData.referer.includes('google') && 
                          !clickData.referer.includes('bing') ? 1 : 0,
        }
      });

      // Update unique visitors count (count distinct IPs for this date)
      if (isPageView) {
        const uniqueIPs = await prisma.click.findMany({
          where: {
            pageId: pageId,
            timestamp: {
              gte: dateStart,
        ...
        await prisma.analytics.update({
          ...
          data: { uniqueVisitors: uniqueIPs.length }
        });
      }
      
      logger.info(`Daily analytics updated for page: ${pageId}, user: ${userId}, date: ${dateStart.toISOString()}`);
    } catch (error) {
      logger.error('Error updating daily analytics:', error);
      // Don't throw - analytics update failure shouldn't break click recording
    }
  }
  ...
  static async getTopPerformingBlocks(pageId, limit = 10, userId) {
    try {
